...
#include "nf2/lib/C/common/nf2util.h"

#define NETFPGA_DEVNAME "nf2c0"
...
//	nfpga->fd = open(NETFPGA_DEVNAME, O_RDWR);
		...
		nf2.device_name = NETFPGA_DEVNAME;

        if (check_iface(&nf2))    //xxxxxx
        {
                exit(1);
        }
        else
        {
        	printf("check_iface OK!!!!\n");dd
        }

        if (openDescriptor(&nf2))  //xxxxxxxxxxx
        {
                exit(1);
        }
        else
        {
        	printf("openDescriptor OK!!!!\n");dd
        }
